...
module.exports = {
  ...
  async execute(message, args, cmd, client, Discord) {
    ...
    if (cmd === "play" || cmd === "p") {
      if (!args.length)
        return message.channel.send("You need to send a song or url video");
      let song = {};

      //const validURL = await play.validate(args[0]);
...
const stop_song = (message, server_queue) => {
  ...
  if (server_queue.songs === undefined) return message.channel.send('No songs in the queue');
